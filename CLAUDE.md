...
- **Fotos**: `node scripts/build-photos.mjs` recorta las miniaturas, los avatares y la foto del grupo de `/community` (deja el original en `public/assets/community/grupo-original.jpg`) con el punto focal ESCRITO de cada foto (`sharp.strategy.attention` falla: dejó la entrevista con Andy Toh convertida en una mesa de centro). `object-position` por foto en `src/data/home.ts`.
